...
opcodes = {
    ...
    0xd0: ['CALLTOKEN',         8, 1, 0],
    0xd1: ['TOKENBALANCE',      2, 1, 0],
    0xd2: ['CALLTOKENVALUE',    0, 1, 0],
    0xd3: ['CALLTOKENID',       0, 1, 0],
...
def pse_add_action(stacklist, orig, string):
	orig += string
	orig += '\n'
	stacklist.append(string)
...
bytecode = open(sys.argv[1]).read()

bytecode.strip('\n')
bytecode.strip(' ')

len_s = int(len(bytecode)/2)
list_nums = []
#i = 0
...
while pc < total_line:
	...
	if c == 0x00:  # STOP
		...
		stack.append(OneOp.create("not ", stack.pop()))
	elif c == 0x16:  # AND
		stack.append(TwoOp.create(stack.pop(), "&", stack.pop()))
	elif c == 0x17:  # OR
		stack.append(TwoOp.create(stack.pop(), "|", stack.pop()))
	...
	elif c == 0x3c:  # EXTCODECOPY
		address = stack.pop()
		memaddr = stack.pop()
		msgaddr = stack.pop()
		num = stack.pop()
		print(TwoOp.create(ArrayIndexRangeCount("memory", memaddr, num), "=", ArrayIndexRangeCount(ObjectIndex(address, "code"), msgaddr, num)))
	elif c == 0x3d:  #RETURNDATASIZE
		stack.append(FuncCall("sizeof", ["msg.data"]))
	elif c == 0x3e:  #RETURNDATACOPY
		stack.pop()
		stack.pop()
		stack.pop()
	elif c == 0x3f:  #EXTCODEHASH
		stack.append(FuncCall("extcodehash", [TwoOp.create("ext.index", "-", stack.pop())]))
	elif c == 0x40:  # BLOCKHASH
		stack.append(FuncCall("blockhash", [TwoOp.create("block.index", "-", stack.pop())]))
	elif c == 0x41:  # COINBASE
		var = stack.pop()
		stack.append(var)
		stack.append(ObjectIndex(var, "beneficiary_address"))
	elif c == 0x42:  # TIMESTAMP
		var = stack.pop()
		stack.append(var)
		stack.append(ObjectIndex(var, "timestamp"))
	elif c == 0x43:  # NUMBER
		var = stack.pop()
		stack.append(var)
		stack.append(ObjectIndex(var, "number"))
	elif c == 0x44:  # DIFFICULTY
		var = stack.pop()
		stack.append(var)
		stack.append(ObjectIndex(var, "difficulty"))
	elif c == 0x45:  # GASLIMIT
		var = stack.pop()
		stack.append(var)
		stack.append(ObjectIndex(var, "gaslimit"))
	elif c == 0x50:  # POP
		stack.pop()
	elif c == 0x51:  # MLOAD
		i = stack.pop()
		...
		stack.append(FuncCall("call_token_value", []))
	elif c == 0xd3:
		stack.append(FuncCall("call_token_id", []))
	elif c == 0xf0:  # CREATE
		value = stack.pop()
